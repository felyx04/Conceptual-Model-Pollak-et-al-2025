...
niterations = 10_000   #100_000   (for 1000yr res: 500_000 + walker_jumps=0.3)
...
def lnprior(parameters):
    aEsi, aEco, aO, ag, ad, taud0, kEsi, kEco, kO, v0, v1, vi, v0_prime, taud0_prime, t2, t1 = parameters
    ...
    if (t1 < t2 < -start_year) and (0 < t1 < t2):   
        return 0.0
    else:
        return -np.inf
...
# Nested sampling via dynesty for optimization
    ...
    def ptform(theta):
        ...
        for i, name in enumerate(names):
            ...
            if name in ['t2', 't1']:  # handle special negative scaling for t1 and t2
                value = high * theta[i]  # maps [0, 1] to [0, -start_year]
            else:
    ...
    def reverse_ptform(params):
        ...
        for i, name in enumerate(names):
            low, high = bounds[name]
            if name in ['t2', 't1']:  # Special handling for t2 and t1
                unit_value = params[i] / high  # Reverse the transformation for [0, -start_year]
            else:
...
# PyMC for optimization
    ...
    bounds = {'lower': [-1_000, -1_000, -1_000, -1_000, -1_000, -1_000, -1_000, -1_000, -1_000, -1_000, -1_000, -1_000, -1_000, -1_000,  1_001,       0],
              'upper': [ 1_000,  1_000,  1_000,  1_000,  1_000,  1_000,  1_000,  1_000,  1_000,  1_000,  1_000,  1_000,  1_000,  1_000, -start_year, 1_000]
              }
    ...
    with pm.Model(coords=coords) as model:
        ...
        # ps = pt.as_tensor_variable([aEsi, aEco, aO, ag, ad, taud0, kEsi, kEco, kO, v0, v1, vi, v0_prime, taud0_prime, t2, t1])
