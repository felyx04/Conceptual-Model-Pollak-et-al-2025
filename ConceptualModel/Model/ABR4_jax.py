...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov 26 10:29:43 2024

@author: pollakf

Note:  - Based on RAMP7_jax.py, but changed for ABR model 
    
       - Switch included to switch between normal and intersection-based model
         (if switch false: normal ABR model /wo intersects)
...
parameter_names = ['aEsi', 'aEco', 'aO', 'ag', 'ad', 'taud', 'kEsi', 'kEco', 'kO', 'v0', 'v1', 'vi', 'ageMPT', 'v0MPT', 'taud_MPT']
...
niterations = 5_000   #100_000   (for 1000yr res: 500_000 + walker_jumps=0.3)
...
def Phi(i, vt, params, state, global_vars, sim_time, time_steps):
    ...
    taud_t = jnp.where(t >= params[12], params[14], params[5])
...
if intersections:
    ...
    def calc_bounds(params, global_vars, time_steps):
        ...
        def body_fn(i, val):
            ...
            v0_t = jnp.where(t >= params[12], params[13], params[9])
    ...
    def Phi_intersect(vt, params, state, global_vars, time_intersect, time_halfsteps):
        ...
        taud_t = jnp.where(time_intersect >= params[12], params[14], params[5])
        taud_t = jnp.where(taud_t == 0, 1e-8, taud_t)
...
# PyMC for optimization
    ...
    bounds = {'lower': [-1_000, -1_000, -1_000, -1_000, -1_000, -1_000, -1_000, -1_000, -1_000, -1_000, -1_000, -1_000,           0,  -1_000, -1_000],
              'upper': [ 1_000,  1_000,  1_000,  1_000,  1_000,  1_000,  1_000,  1_000,  1_000,  1_000,  1_000,  1_000,  -start_year,  1_000,  1_000]
              }
    ...
    with pm.Model(coords=coords) as model:
        ...
        # sigma_taud = pm.HalfNormal('sigma_taud', sigma=10)
        ...
        # sigma_ageMPT = pm.HalfNormal('sigma_ageMPT', sigma=10)
        # sigma_v0MPT = pm.HalfNormal('sigma_v0MPT', sigma=10)
        # sigma_taud_MPT = pm.HalfNormal('sigma_taud_MPT', sigma=100)
        ...
        # taud = pm.Normal("taud", mu=StartPosition_dict['taud'], sigma=sigma_taud, initval=StartPosition_dict['taud']) 
        ...
        # ageMPT = pm.Normal("ageMPT", mu=StartPosition_dict['ageMPT'], sigma=sigma_ageMPT, initval=StartPosition_dict['ageMPT']) 
        # v0MPT = pm.Normal("v0MPT", mu=StartPosition_dict['v0MPT'], sigma=sigma_v0MPT, initval=StartPosition_dict['v0MPT']) 
        # taud_MPT = pm.Normal("taud_MPT", mu=StartPosition_dict['taud_MPT'], sigma=sigma_taud_MPT, initval=StartPosition_dict['taud_MPT']) 
        ...
        # taud = pm.Uniform("taud", lower=-1000, upper=1000, initval=StartPosition_dict['taud']) 
        ...
        # ageMPT = pm.Uniform("ageMPT", lower=-1000, upper=1000, initval=StartPosition_dict['ageMPT']) 
        # v0MPT = pm.Uniform("v0MPT", lower=-1000, upper=1000, initval=StartPosition_dict['v0MPT']) 
        # taud_MPT = pm.Uniform("taud_MPT", lower=1001, upper=-start_year, initval=StartPosition_dict['taud_MPT']) 
        
        # ps = pt.as_tensor_variable([aEsi, aEco, aO, ag, ad, taud, kEsi, kEco, kO, v0, v1, vi, ageMPT, v0MPT, taud_MPT])
...
if gap==None:
    ...
    # plt.savefig('../Plots/RAMP_5.png', dpi=500, bbox_inches='tight')
    plt.show()
