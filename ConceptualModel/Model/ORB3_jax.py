...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov 28 10:06:59 2024

@author: pollakf

Note: - Based on RAMP7_jax., but changed for GRAD model
...
parameter_names = ['aEsi', 'aEco', 'aO', 'ag', 'ad', 'tau_d', 'kEsi', 'kEco', 'kO', 'v0', 'v1', 'vi']
...
def Phi(i, vt, params, state, global_vars, sim_time, time_steps):
    ...
    dvdt = jnp.where(state == 0,
                     ...
                     -params[0] * Esi[i] - params[1] * Eco[i] - params[2] * EnO[i] + params[4] - vt / params[5])
...
if intersections:
    ...
    def calc_bounds(params, global_vars, time_steps):
        ...
        v0s = params[9]*jnp.zeros(time_steps+1)
        ...
        def body_fn(i, val):
            ...
            # Update bounds
            v0_bounds = v0_bounds.at[i].set(params[9] - params[6] * Esi[2*i] - params[7] * Eco[2*i] - params[8] * EnO[2*i])
    ...
    def Phi_intersect(vt, params, state, global_vars, time_intersect, time_halfsteps):
        ...
        dvdt = jnp.where(state == 0,
                         ...
                         -params[0] * Esi_intersect - params[1] * Eco_intersect - params[2] * EnO_intersect + params[4] - vt/params[5])
...
# PyMC for optimization
    ...
    bounds = {'lower': [-1_000, -1_000, -1_000, -1_000, -1_000, -1_000, -1_000, -1_000, -1_000, -1_000, -1_000, -1_000],
              'upper': [ 1_000,  1_000,  1_000,  1_000,  1_000,  1_000,  1_000,  1_000,  1_000,  1_000,  1_000,  1_000]
              }
    ...
    with pm.Model(coords=coords) as model:
        ...
        # ps = pt.as_tensor_variable([aEsi, aEco, aO, ag, ad, tau_d, kEsi, kEco, kO, v0, v1, vi])
